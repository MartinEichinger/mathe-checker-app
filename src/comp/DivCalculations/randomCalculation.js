...
export function getCalculationSmall1x1(values, checks) {
  ...
  const taskArr = ["plus"];
  if (checks[1] === true) {taskArr.push("minus")};
  if (checks[2] === true) {taskArr.push("mal")};
  if (checks[3] === true) {taskArr.push("geteilt")};
  ...
  return fetch(url, fetchOptions).then(response => {
    ...
    return response.json().then(jsonResponse => {
      console.log('randomCalc/getCalc/jsonResponse: ', jsonResponse);
...
export function randomCalculationRational(values, checks) {
  ...
  if (opSw1 !== 1) {num1 = -num1};
  if (opSw3 !== 1) {num2 = -num2};
